...
def fetch_ts(data_dir, clean=False):
    ...
    top_ten_countries =  np.array(list(table.index))[:10] #dump the countries to a csv file
    if not os.path.isfile(os.path.join(data_dir,"country","country.csv")):
        pd.DataFrame(top_ten_countries,columns=['top_ten_countries']).to_csv(os.path.join(data_dir,"country","country.csv"),index=False)
...
def engineer_features(df,training=True):
    ...
    previous =[7, 14, 28, 70]  #[1, 7, 14, 21, 28]
    ...
    for d,day in enumerate(dates):
        ...
        eng_features['recent_purchases'].append(df[mask]['purchases'].mean())
